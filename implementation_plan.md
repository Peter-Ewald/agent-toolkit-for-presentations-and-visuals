...
1. **Binding validation hook.** A script that checks every Excalidraw arrow's bindings are
...
2. **Environment setup script.** A one-shot `uv sync && uv run playwright install chromium`
...
3. **Harness finalization.** Remove the temporary `@~/Projects/agents/CLAUDE.md` import from
...
- Add CI or a local verification script that catches broken references between workflow docs
  and capability files (this wave's manual cross-reference sweep found several stale paths that
  had drifted silently).
